#!/usr/bin/env bash
#
# Build tsar binaries for all platforms locally.
...
#     tsar-darwin-arm64.tar.gz
#     tsar-darwin-x64.tar.gz
#     tsar-linux-x64.tar.gz
#     tsar-linux-arm64.tar.gz
#     tsar-windows-x64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == "windows-x64" ]]; then
        bun build --compile --external koffi --target=bun-$platform ./dist/bun/cli.js --outfile binaries/$platform/tsar.exe
    else
        bun build --compile --external koffi --target=bun-$platform ./dist/bun/cli.js --outfile binaries/$platform/tsar
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == "windows-x64" ]]; then
        # Windows (zip)
        echo "Creating tsar-$platform.zip..."
        (cd $platform && zip -r ../tsar-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating tsar-$platform.tar.gz..."
        mv $platform tsar && tar -czf tsar-$platform.tar.gz tsar && mv tsar $platform
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf $platform
    if [[ "$platform" == "windows-x64" ]]; then
        mkdir -p $platform && (cd $platform && unzip -q ../tsar-$platform.zip)
    else
        tar -xzf tsar-$platform.tar.gz && mv tsar $platform
    fi
done
