 ...
 *   --analyze      Spawn tsar subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("tsar", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn tsar: ${err.message}`));
...
async function main() {
	...
	const globalAgentsMd = join(homedir(), ".tsar/agent/AGENTS.md");
