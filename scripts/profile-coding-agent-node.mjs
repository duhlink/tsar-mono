...
const agentDirEnvName = "TSAR_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "TSAR_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific TSAR_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force TSAR_OFFLINE=1 / TSAR_SKIP_VERSION_CHECK=1
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "tsar-startup-benchmark-")) : undefined;
