...
```bash
npm install -g @tsar/pods
```

## What is tsar?

`tsar` simplifies running large language models on remote GPU pods. It automatically:
...
export TSAR_API_KEY=your_api_key              # Any string you want for API authentication

# Setup a DataCrunch pod with NFS storage (models path auto-extracted)
tsar pods setup dc1 "ssh root@1.2.3.4" \
...
tsar start Qwen/Qwen2.5-Coder-32B-Instruct --name qwen

# Send a single message to the model
tsar agent qwen "What is the Fibonacci sequence?"
...
export OPENAI_API_KEY=$TSAR_API_KEY
```
...
```bash
tsar pods setup <name> "<ssh>" [options]        # Setup new pod
...
tsar pods                                       # List all configured pods
tsar pods active <name>                         # Switch active pod
tsar pods remove <name>                         # Remove pod from local config
tsar shell [<name>]                             # SSH into pod
tsar ssh [<name>] "<command>"                   # Run command on pod
```
...
```bash
tsar start <model> --name <name> [options]  # Start a model
...
tsar stop [<name>]          # Stop model (or all if no name given)
tsar list                   # List running models with status
tsar logs <name>            # Stream model logs (tail -f)
```

### Agent & Chat Interface

```bash
tsar agent <name> "<message>"               # Single message to model
tsar agent <name> "<msg1>" "<msg2>"         # Multiple messages in sequence
tsar agent <name> -i                        # Interactive chat mode
tsar agent <name> -i -c                     # Continue previous session

# Standalone OpenAI-compatible agent (works with any API)
tsar-agent --base-url http://localhost:8000/v1 --model llama-3.1 "Hello"
tsar-agent --api-key sk-... "What is 2+2?"  # Uses OpenAI by default
tsar-agent --json "What is 2+2?"            # Output event stream as JSONL
tsar-agent -i                                # Interactive mode
```
...
`tsar` includes predefined configurations for popular agentic models, so you do not have to specify `--vllm` arguments manually. `tsar` will also check if the model you selected can actually run on your pod with respect to the number of GPUs and available VRAM. Run `tsar start` without additional arguments to see a list of predefined models that can run on the active pod.
...
tsar start Qwen/Qwen3-Coder-30B-A3B-Instruct --name qwen3

# Qwen3-Coder-480B - State-of-the-art on 8xH200 (data-parallel mode)
tsar start Qwen/Qwen3-Coder-480B-A35B-Instruct-FP8 --name qwen-480b
```

### GPT-OSS Models
```bash
# Requires special vLLM build during setup
tsar pods setup gpt-pod "ssh root@1.2.3.4" --models-path /workspace --vllm gpt-oss

# GPT-OSS-20B - Fits on 16GB+ VRAM
tsar start openai/gpt-oss-20b --name gpt20

# GPT-OSS-120B - Needs 60GB+ VRAM
tsar start openai/gpt-oss-120b --name gpt120
```

### GLM Models
```bash
# GLM-4.5 - Requires 8-16 GPUs, includes thinking mode
tsar start zai-org/GLM-4.5 --name glm

# GLM-4.5-Air - Smaller version, 1-2 GPUs
tsar start zai-org/GLM-4.5-Air --name glm-air
```
...
tsar start deepseek-ai/DeepSeek-V3 --name deepseek --vllm \
  --tensor-parallel-size 4 --trust-remote-code

# Mistral with pipeline parallelism
tsar start mistralai/Mixtral-8x22B-Instruct-v0.1 --name mixtral --vllm \
  --tensor-parallel-size 8 --pipeline-parallel-size 2

# Any model with specific tool parser
tsar start some/model --name mymodel --vllm \
...
tsar pods setup dc1 "ssh root@instance.datacrunch.io" \
  --mount "sudo mount -t nfs -o nconnect=16 nfs.fin-02.datacrunch.io:/your-pseudo /mnt/hf-models"
...
```bash
# With network volume
tsar pods setup runpod "ssh root@pod.runpod.io" --models-path /runpod-volume

# Or use workspace (persists with pod but not shareable)
tsar pods setup runpod "ssh root@pod.runpod.io" --models-path /workspace
```
...
When running multiple models, tsar automatically assigns them to different GPUs:
```bash
tsar start model1 --name m1  # Auto-assigns to GPU 0
tsar start model2 --name m2  # Auto-assigns to GPU 1
tsar start model3 --name m3  # Auto-assigns to GPU 2
```
...
tsar start Qwen/Qwen2.5-Coder-32B-Instruct --name qwen --gpus 1

# Run GLM-4.5 on 8 GPUs (if it has an 8-GPU config)
tsar start zai-org/GLM-4.5 --name glm --gpus 8
```
...
tsar start meta-llama/Llama-3.1-70B-Instruct --name llama70b --vllm \
  --tensor-parallel-size 4

# Specific GPU count
tsar start Qwen/Qwen3-Coder-480B-A35B-Instruct-FP8 --name qwen480 --vllm \
...
`tsar` includes a standalone OpenAI-compatible agent that can work with any API:

```bash
# Install globally to get tsar-agent command
npm install -g @tsar/pods

# Use with OpenAI
tsar-agent --api-key sk-... "What is machine learning?"

# Use with local vLLM
tsar-agent --base-url http://localhost:8000/v1 \
...
tsar-agent --continue "Follow up question"

# Custom system prompt
tsar-agent --system-prompt "You are a Python expert" "Write a web scraper"

# Use responses API (for GPT-OSS models)
tsar-agent --api responses --model openai/gpt-oss-20b "Hello"
```
...
`tsar` automatically configures appropriate tool calling parsers for known models:
...
```bash
tsar start model --name mymodel --vllm --disable-tool-call-parser
```
...
tsar start Qwen/Qwen2.5-Coder-32B-Instruct --name coder \
...
Sessions are stored in `~/.tsar/sessions/` organized by project path and include:
...
```bash
tsar-agent --api-key sk-... --json "What is 2+2?"
```
...
- Check CUDA compatibility with `tsar ssh "nvidia-smi"`
...
```bash
# Good
tsar agent qwen "What is this file about?"
...
tsar start model --name test --pod dev-pod
tsar list --pod prod-pod
tsar stop test --pod dev-pod
```

### Custom vLLM Arguments
```bash
# Pass any vLLM argument after --vllm
tsar start model --name custom --vllm \
  --quantization awq \
...
tsar ssh "watch -n 1 nvidia-smi"

# Check model downloads
tsar ssh "du -sh ~/.cache/huggingface/hub/*"

# View all logs
tsar ssh "ls -la ~/.vllm_logs/"
...
- `OPENAI_API_KEY` - Used by `tsar-agent` when no `--api-key` provided
