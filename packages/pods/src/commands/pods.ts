...
export const setupPod = async (
	...
	const vllmApiKey = process.env.TSAR_API_KEY;
	...
	if (!vllmApiKey) {
		console.error(chalk.red("ERROR: TSAR_API_KEY environment variable is required"));
		console.error("Set an API key: export TSAR_API_KEY=your_api_key_here");
		process.exit(1);
	}
