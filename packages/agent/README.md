...
Stateful agent with tool execution and event streaming. Built on `@tsar/ai`.

## Installation

```bash
npm install @tsar/agent-core
```
...
```typescript
import { Agent, streamProxy } from "@tsar/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@tsar/agent-core";
