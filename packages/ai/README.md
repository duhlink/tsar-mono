...
TypeBox exports are re-exported from `@tsar/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@tsar/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@tsar/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@tsar/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@tsar/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@tsar/ai';
...
```typescript
import { getModel, stream } from '@tsar/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@tsar/ai';
...
```typescript
import { Model, stream } from '@tsar/ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@tsar/ai';
...
```typescript
import { getModel, complete, Context } from '@tsar/ai';
...
```typescript
import { Context, getModel, complete } from '@tsar/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@tsar/ai/oauth` entry point in Node.js.
...
| Azure OpenAI | `AZURE_OPENAI_API_KEY` + `AZURE_OPENAI_BASE_URL` or `AZURE_OPENAI_RESOURCE_NAME` (optional `AZURE_OPENAI_ATSAR_VERSION`, `AZURE_OPENAI_DEPLOYMENT_NAME_MAP` like `model=deployment,model2=deployment2`) |
...
Set `TSAR_AI_ANTIGRAVITY_VERSION` to override the Antigravity User-Agent version when Google updates their requirements:

```bash
export TSAR_AI_ANTIGRAVITY_VERSION="1.23.0"
```

#### Cache Retention

Set `TSAR_CACHE_RETENTION=long` to extend prompt cache retention:

| Provider | Default | With `TSAR_CACHE_RETENTION=long` |
|----------|---------|-------------------------------|
...
```typescript
import { getEnvApiKey } from '@tsar/ai';
...
```bash
npx @tsar/ai login              # interactive provider selection
npx @tsar/ai login anthropic    # login to specific provider
npx @tsar/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@tsar/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
```typescript
import { loginGitHubCopilot } from '@tsar/ai/oauth';
...
```typescript
import { getModel, complete } from '@tsar/ai';
import { getOAuthApiKey } from '@tsar/ai/oauth';
...
**Azure OpenAI (Responses)**: Uses the Responses API only. Set `AZURE_OPENAI_API_KEY` and either `AZURE_OPENAI_BASE_URL` or `AZURE_OPENAI_RESOURCE_NAME`. Use `AZURE_OPENAI_ATSAR_VERSION` (defaults to `v1`) to override the API version if needed. Deployment names are treated as model IDs by default, override with `azureDeploymentName` or `AZURE_OPENAI_DEPLOYMENT_NAME_MAP` using comma-separated `model-id=deployment` pairs (for example `gpt-4o-mini=my-deployment,gpt-4o=prod`). Legacy deployment-based URLs are intentionally unsupported.
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@tsar/ai`
