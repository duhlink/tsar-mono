...
export class TUI extends Container {
	...
	private showHardwareCursor = process.env.TSAR_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.TSAR_CLEAR_ON_SHRINK === "1"; // Clear empty rows when content shrinks (default: off)
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.TSAR_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(os.homedir(), ".tsar", "agent", "tsar-debug.log");
		...
		// Configurable via setClearOnShrink() or TSAR_CLEAR_ON_SHRINK=0 env var
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(os.homedir(), ".tsar", "agent", "tsar-crash.log");
				const crashData = [
		...
		if (process.env.TSAR_TUI_DEBUG === "1") {
