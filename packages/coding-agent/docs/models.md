...
Some OpenAI-compatible servers do not understand the `developer` role used for reasoning-capable models. For those providers, set `compat.supportsDeveloperRole` to `false` so tsar sends the system prompt as a `system` message instead. If the server also does not support `reasoning_effort`, set `compat.supportsReasoningEffort` to `false` too.
...
For `models.json`, shell commands are resolved at request time. tsar intentionally does not apply built-in TTL, stale reuse, or recovery logic for arbitrary commands. Different commands need different caching and failure strategies, and tsar cannot infer the right one.
...
| `reasoningEffortMap` | Map tsar thinking levels to provider-specific `reasoning_effort` values |
