> tsar can create themes. Ask it to build one for your setup.
...
Tsar loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.tsar/agent/themes/*.json`
- Project: `.tsar/themes/*.json`
...
On first run, tsar detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.tsar/agent/themes
vim ~/.tsar/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, tsar reloads it automatically for immediate visual feedback.
...
Tsar uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, tsar falls back to the nearest approximation.
