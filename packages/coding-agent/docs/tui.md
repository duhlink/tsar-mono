> tsar can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@tsar/tui`](https://github.com/badlogic/pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@tsar/tui";
...
```typescript
import { Container, type Focusable, Input } from "@tsar/tui";
...
Import from `@tsar/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@tsar/tui";
```
...
```typescript
import { matchesKey, Key } from "@tsar/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@tsar/tui";
...
```typescript
import { getMarkdownTheme } from "@tsar/coding-agent";
import { Markdown } from "@tsar/tui";
...
Set `TSAR_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
TSAR_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@tsar/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@tsar/coding-agent";
import { DynamicBorder } from "@tsar/coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@tsar/tui";
...
```typescript
import { BorderedLoader } from "@tsar/coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@tsar/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@tsar/coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@tsar/tui";
...
import { matchesKey, truncateToWidth } from "@tsar/tui";
