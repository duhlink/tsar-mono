...
```
~/.tsar/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.tsar/agent/sessions/`.

Tsar also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, tsar uses the `trash` CLI to avoid permanent deletion.
...
For TypeScript definitions in your project, inspect `node_modules/@tsar/coding-agent/dist/` and `node_modules/@tsar/ai/dist/`.
...
### Base Message Types (from tsar-ai)
...
### Extended Message Types (from tsar-coding-agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if tsar-generated (legacy field name)
