> tsar can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to tsar's agent capabilities. Use it to embed tsar in other applications, build custom interfaces, or integrate with automated workflows.
...
```typescript
import { createAgentSession } from "@tsar/coding-agent";
...
The `Agent` class (from `@tsar/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.tsar/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.tsar/extensions/`)
- Project skills:
  - `.tsar/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.tsar/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.tsar/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@tsar/coding-agent";
...
// Default: uses ~/.tsar/agent/auth.json and ~/.tsar/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = new ModelRegistry(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@tsar/coding-agent";
...
- If you omit `tools`, tsar automatically creates them with the correct `cwd`
...
import { createAgentSession, type ToolDefinition } from "@tsar/coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.tsar/agent/extensions/`, `.tsar/extensions/`, and settings.json extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@tsar/coding-agent";
...
```typescript
import { createAgentSession, SessionManager } from "@tsar/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@tsar/coding-agent";
...
1. Global: `~/.tsar/agent/settings.json`
2. Project: `<cwd>/.tsar/settings.json`
...
```typescript
import { createAgentSession, InteractiveMode } from "@tsar/coding-agent";
...
```typescript
import { createAgentSession, runPrintMode } from "@tsar/coding-agent";
...
```typescript
import { createAgentSession, runRpcMode } from "@tsar/coding-agent";
