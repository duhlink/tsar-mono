...
import type { Api, Context, Model, OpenAICompletionsCompat } from "@tsar/ai";
import { getApiProvider } from "@tsar/ai";
import { getOAuthProvider } from "@tsar/ai/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `tsar-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
