...
import { getModel, type OAuthCredentials, type OAuthProvider } from "@tsar/ai";
...
// ============================================================================
// OAuth API key resolution from ~/.tsar/agent/auth.json
// ============================================================================
...
/**
 * Resolve API key for a provider from ~/.tsar/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.tsar/agent/auth.json
 */
...
/** Path to the real tsar agent config directory */
export const TSAR_AGENT_DIR = join(homedir(), ".tsar", "agent");

/**
 * Get an AuthStorage instance backed by ~/.tsar/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `tsar-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
